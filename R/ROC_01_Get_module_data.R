...
get_module_data <- function(train_set, test_set, sig_v){
  ...
  if(c("SampleID", "ClassNote") %notin% (train_set@phenoData@data %>% colnames) %>% sum|
     c("SampleID", "ClassNote") %notin% (test_set@phenoData@data %>% colnames) %>% sum){
    stop("
    phenodata df must have two column： SampleID and ClassNote（for group）
         表型数据@phenoData@data中的列名需要包含'SampleID'和 'ClassNote'")
  }
...
#' @title get random seeds for random selection
#' @param Today the date for seed
#' @param length the num of seeds needed
#' @examples
#' \dontrun{
#' seeds <- get_seeds(Today = Sys.Date() + 3, length = 10)
#' }
#' @return seeds
#' @export
get_seeds <- function(Today = Sys.Date(), length = 100){
  seeds <- (Today - 1:length) %>% gsub("-", "", .)
  return(seeds)
}
